...
/**
 * Werkdagen van `fromIso` tot en met `toIso`, beide meegeteld. De ankerdag zelf
 * is werkdag 1 — daarom inclusief. Valt het anker in het weekend, dan staat de
 * teller op 0 tot de eerstvolgende maandag.
 */
 ...
 */
export function nthWorkdayFrom(fromIso: string, n: number): string | null {
  if (n < 1) return null
  let count = 0
  let day = fromIso
...
export function buildCycle(input: CycleInput): LoopgangCycle {
  ...
  const workday = countWorkdays(anchor, today)
  const calendarDay = Math.max(0, daysBetween(anchor, today) + 1)

  const meetingReminderStart = nthWorkdayFrom(anchor, MEETING_WORKDAY)
  const invoiceDueDate = nthWorkdayFrom(anchor, INVOICE_WORKDAY)
  const meetingWindow = {
    from: addDays(anchor, MEETING_WINDOW_FROM - 1),
    to: addDays(anchor, MEETING_WINDOW_TO - 1),
  }
