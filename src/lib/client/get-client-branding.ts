...
export const getClientBranding = cache(async () => {
  ...
  const { data: client, error } = await supabase
    .from('clients')
    .select('id, company_name, primary_color, logo_url, meeting_url, inbox_url, inbox_visible, chat_inbox_visible, lead_inbox_visible, lead_inbox_customer_id, email_signature, onboarding_status')
    .eq('id', clientId)
    .single()
