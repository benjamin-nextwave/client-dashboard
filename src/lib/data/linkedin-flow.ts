 ...
 *   - `charLimit`: optional soft limit (200 for the connection note, 400
 *     for the first message after accept). Not enforced server-side; the
 *     UI surfaces a counter so the operator stays inside LinkedIn limits.
 */
...
const SELECT_COLS =
  'id, linkedin_flow_enabled, linkedin_message_day_plus_1, linkedin_message_day_plus_4, linkedin_message_day_plus_9, linkedin_message_day_plus_14, linkedin_flow_published_at, linkedin_flow_approved_at'
...
export async function getLinkedInFlowState(
  clientId: string
