...
export function SidebarNav({ companyName, logoUrl, signOutAction, meetingUrl, inboxUrl, inboxVisible }: SidebarNavProps) {
  const navItems = [
    {
      href: '/dashboard',
      label: 'Overzicht',
    ...
    {
      href: meetingUrl || 'https://meetings.nextwave.nl',
      label: 'Afspraken',
      external: true,
      icon: (
        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
      ...
      <nav className="flex-1 space-y-1 px-3 py-4">
        {navItems.map((item) => (
          <NavItem
            ...
            label={item.label}
            icon={item.icon}
            external={'external' in item ? (item as { external?: boolean }).external : undefined}
          />
        ))}
      </nav>
