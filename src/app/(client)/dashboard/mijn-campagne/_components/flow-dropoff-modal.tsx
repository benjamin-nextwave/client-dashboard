        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {reasons.length === 0 ? (
            <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 px-4 py-8 text-center text-sm text-gray-400">
              Er zijn nog geen specifieke afhaak-redenen ingesteld.
            </div>
          ) : (
            <>
              <div className="mb-3 text-[10px] font-bold uppercase tracking-wider text-gray-500">
                {reasons.length} {reasons.length === 1 ? 'reden' : 'redenen'}
              </div>
          ...
          <div className="mt-5 rounded-xl border border-rose-100 bg-rose-50/40 px-4 py-3 text-xs text-rose-900">
            Wanneer een lead via één van bovenstaande redenen afhaakt, eindigt deze tak van de
            campagne en{' '}
            {respLabel ? (
              <>
                wordt de afhandeling opgepakt door <strong>{respLabel.toLowerCase()}</strong>.
              </>
            ) : (
              <>volgt geen verdere actie meer.</>
            )}
          </div>
        </div>
